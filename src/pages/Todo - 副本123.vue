<template>
    <div>
        ...
        <div class="todos" id="todos">
            ...
            <div class="list-empty" v-if="!loadAble">就这些了~</div>
            </div>
        </div>
        <div class="tool-bar">
            <div class="checks" :class="{'border-red': status==0, 'border-orange': status==1, 'border-blue': status==2, 'border-disabled': !checked_ids.length}">
                <div class="checkbox" :class="{'checked': status==0&&checked_ids.length}" style="margin-left: 8px;" @click="changeCheckStatus(0)">
                    <label for="#check_status1"></label>{{status==0&&checked_ids.length?'反选':'全选'}}
                    <input type="checkbox" id="check_status1" hidden>
                </div>

                <div class="checkbox" :class="{'checked': status==1&&checked_ids.length}" style="margin-left: 8px;" @click="changeCheckStatus(1)" v-if="doneAndUndone.done">
                ...
                <div class="checkbox" :class="{'checked': status==2&&checked_ids.length}" style="margin-left: 8px;" @click="changeCheckStatus(2)" v-if="doneAndUndone.undone">
...
.checkbox.checked label::after { content: ''; position: absolute; left: 5px; width: 20px; height: 8px; border-left: 2px solid red; border-bottom: 2px solid red; transform: rotate(-40deg); border-bottom-left-radius: 3px; }
